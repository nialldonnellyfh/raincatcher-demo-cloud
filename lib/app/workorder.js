...
var SheetStore = require('../storage/google-sheet-store');
...
module.exports = function(mediator) {
  ...
  var workorderSheetStore = new SheetStore('workorders', 'iidn3tvprs62asdebat5m3eh');

  workorderDataTopiocs
    .on('create', function(workorder) {
      return workorderSheetStore.create(workorder);
    })
    .on('read', function(id) {
      return workorderSheetStore.read(id);
    })
    .on('update', function(workorder) {
      return workorderSheetStore.update(workorder);
    })
    .on('delete', function(id) {
      return workorderSheetStore.remove(id);
    })
    .on('list', function(filter) {
      return workorderSheetStore.list(filter);
    });
};
